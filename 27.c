...
int hash_function(int key) { return key % HASH_MOD; }
...
int update_graph(grafo_t *GRAPH, char command) {
  ...
  if (command == ROTTAMA_AUTO) {
    ...
    if (current_station != NULL) {
      if (tree_search(current_station->stazione,
                      current_station->stazione->root,
                      command_number) != current_station->stazione->nil) {
        nodo_albero_t *deleted_auto =
            rb_delete(current_station->stazione, command_number);
        free(deleted_auto);
        printf("rottamata\n");
        return 0;
      }
  ...
  while (queue->head != NULL) {
    ...
    if (curr_lista != NULL) {
      ...
      for (i = len - 1; i >= 0; i--) {
        ...
        if (nodi[i] > curr_grafo->key &&
            nodi[i] - curr_grafo->key <= curr_grafo->stazione->max) {
      ...
      while (reachable_tail != -1) {
        if (reachable[reachable_tail] > nodi[0] &&
            reachable[reachable_tail] <= nodi[len - 1]) {
          nodo_grafo_t *v_grafo =
              graph_search(GRAPH, reachable[reachable_tail]);
  ...
  while (queue->head != NULL) {
    ...
    if (curr_lista != NULL) {
      ...
      for (i = len - 1; i >= 0; i--) {
        ...
        if (nodi[i] > curr_grafo->key &&
            nodi[i] - curr_grafo->key <= i_grafo->stazione->max) {
          reachable[reachable_tail] = nodi[i];
          reachable_tail++;
        }
      }
    ...
    if (strcmp(command, "aggiungi-auto") == 0) {
      ...
      if (end_num > start_num) {
        ...
        for (int m = 0; m < HASH_SIZE; m++) {
          if (GRAPH->items[m] != NULL && GRAPH->items[m]->key > start_num &&
              GRAPH->items[m]->key < end_num) {
